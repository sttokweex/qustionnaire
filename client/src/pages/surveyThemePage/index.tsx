...
const SurveyThemePage: React.FC<SurveyThemePageProps> = ({
  ...
  if (!title) {
    return <div>Error: title is missing in the URL.</div>; // Error message for missing title
  }
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      await addSurveyMutation.mutateAsync(formData); // Ожидаем выполнения запроса
      refetchThemes();
    } catch (error: any) {
      throw new Error(
  ...
  return (
    <div className="h-screen w-full bg-gray-50">
      ...
      {isLoading && (
        <div className="text-gray-600 text-center">Loading surveys...</div> // Loading message
      )}
      {userData.role === 'admin' && (
